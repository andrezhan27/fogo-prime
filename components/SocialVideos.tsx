  ...
  return (
    ...
    >
      <div className="mx-auto max-w-[76rem]">
        ...
        <Reveal
          delay={0.08}
          className="mt-10 grid overflow-hidden border border-[#cdbfae] bg-[#1b1713] text-[#f7f0e5] lg:grid-cols-[1fr_22rem]"
        >
            ...
            <div className="mt-8 grid max-w-xl gap-3 sm:grid-cols-3">
              {socialLinks.map(({ key, href, icon: Icon }) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="group flex min-h-14 items-center justify-between border border-white/15 px-4 text-sm font-semibold transition-colors hover:border-[#dca51d] hover:text-[#f2bd39] focus-visible:outline-2 focus-visible:outline-[#f2bd39]"
                >
                  <span className="flex items-center gap-3">
                    <Icon
                      aria-hidden="true"
                      className="size-4 text-[#f2bd39]"
                      strokeWidth={1.8}
                    />
                    {copy.social[key]}
                  </span>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="group block border-t border-white/12 focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[#f2bd39] lg:border-l lg:border-t-0"
            aria-label={`${copy.social.watch} TikTok: ${copy.social.videoCaption}`}
          >
            <div className="relative mx-auto aspect-[9/12] h-full min-h-[25rem] w-full max-w-[22rem] overflow-hidden bg-[#0b0b0a] lg:aspect-auto">
              <Image
                src="/images/tiktok-thumbnail.webp"
                alt={copy.social.videoCaption}
                fill
              ...
              <div className="absolute inset-x-0 bottom-0 p-6">
                <span className="ui-label text-[#f2bd39]">
                  TikTok · @ondeirempt
                </span>
                <p className="mt-2 text-lg font-semibold leading-snug">
                  {copy.social.videoCaption}
                </p>
              </div>
            </div>
          </a>
