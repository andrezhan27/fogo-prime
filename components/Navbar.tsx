      ...
      <div className="mx-auto flex h-[4.75rem] max-w-[76rem] items-center justify-between px-6 sm:px-10 lg:h-[5.5rem] lg:px-16 xl:px-20">
        ...
        <nav aria-label="Navegação principal" className="hidden items-center gap-7 xl:flex">
          {links.map(([key, href]) => (
            <a
              key={key}
              href={href}
              className="text-[0.78rem] font-semibold uppercase tracking-[0.12em] text-white/75 transition-colors hover:text-[#f2bd39] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#f2bd39]"
            >
              {copy.nav[key]}
            </a>
          ))}
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <div className="hidden items-center gap-1 sm:flex" aria-label="Language">
            {(['pt', 'en'] as const).map((item) => (
              <button
                ...
                className={`min-h-11 min-w-11 text-xs font-bold uppercase tracking-[0.14em] transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#f2bd39] ${
                  language === item ? 'text-[#f2bd39]' : 'text-white/55 hover:text-white'
                }`}
          ...
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            ...
            <SheetContent className="w-[min(90vw,28rem)] border-l border-white/10 bg-[#0b0b0a] text-[#f7f0e5]" showCloseButton>
              <SheetHeader className="border-b border-white/10 px-6 py-7 text-left">
                <SheetTitle className="font-heading text-3xl text-[#f7f0e5]">Fogo Prime</SheetTitle>
                <SheetDescription className="text-[#a89f92]">Rodízio brasileiro · Lisboa</SheetDescription>
              </SheetHeader>
              <nav className="flex flex-col px-6 py-5" aria-label="Navegação móvel">
                {links.map(([key, href], index) => (
                  <a key={key} href={href} onClick={() => setMobileOpen(false)} className="flex min-h-14 items-center border-b border-white/10 font-heading text-2xl text-[#f7f0e5] focus-visible:outline-2 focus-visible:outline-[#f2bd39]">
                      <span className="mr-4 text-xs text-[#dca51d]">0{index + 1}</span>
                      {copy.nav[key]}
                  </a>
                ))}
              </nav>
                ...
                <div className="flex gap-2">
                  {(['pt', 'en'] as const).map((item) => (
                    <button
                      ...
                      type="button"
                      onClick={() => setLanguage(item)}
                      className={`min-h-11 min-w-14 border text-xs font-bold uppercase tracking-[0.14em] ${
                        language === item
